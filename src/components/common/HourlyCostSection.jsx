...
function HourlyCostSection({ value = [], onChange }) {
  ...
  return <Box>
    ...
    <Stack spacing={1.5}>{rows.map((row, index) => <Box key={index} sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr 36px' }, gap: 1.5, alignItems: 'center' }}>
      <TextField required type="date" label="Date" value={row.date} onChange={(event) => update(index, 'date', event.target.value)} slotProps={{ inputLabel: { shrink: true } }} />
      <TextField required type="number" label="Per Hour Cost" value={row.perHourCost} onChange={(event) => update(index, 'perHourCost', event.target.value === '' ? '' : Number(event.target.value))} slotProps={{ htmlInput: { min: 0, step: '0.01' } }} />
