...
import { Box, IconButton, InputAdornment, MenuItem, Select, Stack, Tab, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Tabs, TextField, Typography } from '@mui/material'
import ChevronLeftRoundedIcon from '@mui/icons-material/ChevronLeftRounded'
import ChevronRightRoundedIcon from '@mui/icons-material/ChevronRightRounded'
import SearchRoundedIcon from '@mui/icons-material/SearchRounded'

function PaginatedTable({ columns, rows, renderRow, minWidth, tabs = [] }) {
  const [page, setPage] = useState(0)
  const [rowsPerPage, setRowsPerPage] = useState(10)
  const [search, setSearch] = useState('')
  const [selectedTab, setSelectedTab] = useState(tabs[1]?.value || tabs[0]?.value || '')
  const query = search.trim().toLowerCase()
  const tabRows = selectedTab ? rows.filter((row) => tabs.find((tab) => tab.value === selectedTab)?.filter(row)) : rows
  const filteredRows = query ? tabRows.filter((row) => Object.values(row).some((value) => typeof value !== 'object' && String(value ?? '').toLowerCase().includes(query))) : tabRows
  const pageCount = Math.max(1, Math.ceil(filteredRows.length / rowsPerPage))
  const currentPage = Math.min(page, pageCount - 1)
  const start = currentPage * rowsPerPage
  return <>
    {tabs.length > 0 && <Tabs value={selectedTab} onChange={(_, value) => { setSelectedTab(value); setPage(0) }} sx={{ px: { xs: 1, sm: 2 }, borderBottom: '1px solid', borderColor: 'divider' }}>{tabs.map((tab) => <Tab key={tab.value} value={tab.value} label={`${tab.label} (${rows.filter(tab.filter).length})`} />)}</Tabs>}
    ...
    {filteredRows.length > 0 && <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr auto', sm: '1fr 1fr 1fr' }, gridTemplateAreas: { xs: '"total rows" "pager pager"', sm: '"total pager rows"' }, alignItems: 'center', columnGap: 2, rowGap: { xs: 1, sm: 0 }, minHeight: 64, px: { xs: 1.5, sm: 2 }, py: { xs: 1.25, sm: 1 }, borderTop: '1px solid', borderColor: 'divider' }}>
      ...
      <Stack direction="row" alignItems="center" sx={{ gridArea: 'pager', justifySelf: 'center', width: 'max-content', flexWrap: 'nowrap', gap: '8px' }}>
        <IconButton size="small" aria-label="Previous page" disabled={currentPage === 0} onClick={() => setPage((value) => value - 1)} sx={{ width: 36, height: 36 }}><ChevronLeftRoundedIcon /></IconButton>
        <Typography sx={{ width: 48, textAlign: 'center', fontSize: 13, lineHeight: '36px', fontWeight: 700, whiteSpace: 'nowrap' }}>{currentPage + 1} / {pageCount}</Typography>
        <IconButton size="small" aria-label="Next page" disabled={currentPage + 1 >= pageCount} onClick={() => setPage((value) => value + 1)} sx={{ width: 36, height: 36 }}><ChevronRightRoundedIcon /></IconButton>
      </Stack>
