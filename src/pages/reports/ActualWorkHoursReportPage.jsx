...
const blank = { voucherNo: '', voucherDate: '', ledgerName: '', employeeId: '', employeeName: '', projectName: '', projectId: '', subProjectId: '', monthDate: '', actualHours: '', actualHoursAmount: '', sheetName: '' }
...
function ActualWorkHoursReportPage() {
  ...
  const download = () => exportExcel('Actual-Hours-Report', 'Actual Hours', ['Voucher No','Voucher Date','Type','Ledger','Emp ID','Employee Name','Project Name','Project ID','Sub Project ID','Month','Actual HRS','Amount','Sheet'], filtered.map((item) => [item.voucherNo, formatDate(item.voucherDate), item.type || 'Actual', item.ledgerName, item.employeeId, item.employeeName, item.projectName, item.projectId, item.subProjectId, formatMonth(item.month), item.actualHours, item.actualHoursAmount ?? item.amount ?? '', item.sheetName]))

  const startEdit = (item) => {
    const monthDate = clean(item.monthDate || item.month).slice(0, 7)
    setEditing(item); setForm({ ...blank, ...item, monthDate, actualHoursAmount: item.actualHoursAmount ?? item.amount ?? '' }); setError('')
  }
  const change = (field, value) => setForm((current) => ({ ...current, [field]: value }))
  const save = async (event) => {
    event.preventDefault()
    const actualHours = Number(form.actualHours), actualHoursAmount = Number(form.actualHoursAmount)
    ...
    if (form.actualHours === '' || !Number.isFinite(actualHours) || actualHours < 0) return setError('Actual HRS must be a valid number greater than or equal to 0.')
    if (form.actualHoursAmount === '' || !Number.isFinite(actualHoursAmount) || actualHoursAmount < 0) return setError('Amount must be a valid number greater than or equal to 0.')
    setSaving(true)
    try {
      await updateRecord('actualWorkHours', editing.id, { voucherNo: clean(form.voucherNo), voucherDate: form.voucherDate, type: 'Actual', ledgerName: clean(form.ledgerName), employeeId: clean(form.employeeId), employeeName: clean(form.employeeName), projectName: clean(form.projectName), projectId: clean(form.projectId), subProjectId: clean(form.subProjectId), month: formatMonth(`${form.monthDate}-01`), monthDate: `${form.monthDate}-01`, actualHours, amount: actualHoursAmount, actualHoursAmount, sheetName: clean(form.sheetName) })
  ...
  return <Stack spacing={3}><PageHeader section="Reports / Actual Work Hours" title="Actual Work Hours Report" description="View and manage imported actual-hour entries by voucher date." />
    ...
    <FormDrawer open={Boolean(editing)} onClose={() => setEditing(null)} title="Edit Actual Work Hours" subtitle="Change the imported record values and save." onSubmit={save} saving={saving} submitLabel="Save Changes" width={760}>
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: 'repeat(2,minmax(0,1fr))' }, gap: 2 }}>
        ...
        <TextField required type="number" label="Amount" value={form.actualHoursAmount} onChange={(e) => change('actualHoursAmount', e.target.value)} slotProps={{ htmlInput: { min: 0, step: '0.01' } }} />
