...
import { Box, Card, Stack, TableCell, TableRow, TextField } from '@mui/material'
...
function ActualWorkHoursReportPage() {
  ...
  const filtered = useMemo(() => records.data.filter((item) => (!fromDate || item.voucherDate >= fromDate) && (!toDate || item.voucherDate <= toDate)).sort((a, b) => String(b.voucherDate).localeCompare(String(a.voucherDate))), [records.data, fromDate, toDate])
