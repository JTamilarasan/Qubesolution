...
const blank = { voucherNo: '', voucherDate: '', type: 'Forecast', ledgerName: '', employeeId: '', employeeName: '', projectName: '', projectId: '', subProjectId: '', monthDate: '', forecastHours: '', forecastAmount: '', sheetName: '' }
const clean = (value) => String(value ?? '').trim()

function ForecastEntriesReportPage() {
  const records = useCollection('forecastEntries')
  ...
  const download = () => exportExcel('Forecast-Entries-Report', 'Forecast Entries', ['Voucher No','Voucher Date','Type','Ledger','Emp ID','Employee Name','Project Name','Project ID','Sub Project ID','Month','Forecast HRS','Amount','Sheet'], filtered.map((item) => [item.voucherNo, formatDate(item.voucherDate), item.type || 'Forecast', item.ledgerName, item.employeeId, item.employeeName, item.projectName, item.projectId, item.subProjectId, formatMonth(item.month), item.forecastHours, item.forecastAmount ?? item.amount ?? '', item.sheetName]))

  const startEdit = (item) => {
    const monthDate = clean(item.monthDate || item.month).slice(0, 7)
    setEditing(item); setForm({ ...blank, ...item, monthDate, forecastAmount: item.forecastAmount ?? item.amount ?? '' }); setError('')
  }
  const change = (field, value) => setForm((current) => ({ ...current, [field]: value }))
  const save = async (event) => {
    event.preventDefault()
    const forecastHours = Number(form.forecastHours), forecastAmount = Number(form.forecastAmount)
    ...
    if (form.forecastHours === '' || !Number.isFinite(forecastHours) || forecastHours < 0) return setError('Forecast HRS must be a valid number greater than or equal to 0.')
    if (form.forecastAmount === '' || !Number.isFinite(forecastAmount) || forecastAmount < 0) return setError('Amount must be a valid number greater than or equal to 0.')
    setSaving(true)
    try {
      await updateRecord('forecastEntries', editing.id, { voucherNo: clean(form.voucherNo), voucherDate: form.voucherDate, type: 'Forecast', ledgerName: clean(form.ledgerName), employeeId: clean(form.employeeId), employeeName: clean(form.employeeName), projectName: clean(form.projectName), projectId: clean(form.projectId), subProjectId: clean(form.subProjectId), month: formatMonth(`${form.monthDate}-01`), monthDate: `${form.monthDate}-01`, forecastHours, amount: forecastAmount, forecastAmount, sheetName: clean(form.sheetName) })
  ...
  return <Stack spacing={3}><PageHeader section="Reports / Forecast Entries" title="Forecast Entries Report" description="View and manage imported forecast-hour entries by voucher date." />
    ...
    <FormDrawer open={Boolean(editing)} onClose={() => setEditing(null)} title="Edit Forecast Entries" subtitle="Change the imported record values and save." onSubmit={save} saving={saving} submitLabel="Save Changes" width={760}>
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: 'repeat(2,minmax(0,1fr))' }, gap: 2 }}>
        ...
        <TextField label="Type" value="Forecast" slotProps={{ input: { readOnly: true } }} /><TextField required label="Ledger" value={form.ledgerName} onChange={(e) => change('ledgerName', e.target.value)} /><TextField required label="Emp ID" value={form.employeeId} onChange={(e) => change('employeeId', e.target.value)} />
        ...
        <TextField required type="number" label="Amount" value={form.forecastAmount} onChange={(e) => change('forecastAmount', e.target.value)} slotProps={{ htmlInput: { min: 0, step: '0.01' } }} />
