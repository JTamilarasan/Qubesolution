...
const fromColumn = (row, names) => {
  const key = Object.keys(row).find((column) => names.some((name) => column.trim().toLowerCase() === name.toLowerCase()))
  return key ? row[key] : ''
}
const monthValue = (value) => {
  if (typeof value === 'number') {
    const parsed = XLSX.SSF.parse_date_code(value)
    if (parsed) return `${parsed.y}-${String(parsed.m).padStart(2, '0')}-01`
  }
...
function OtherEntriesImportPage() {
  ...
  const entries = useCollection('otherEntries')
  ...
  const validate = () => {
    ...
    const checked = raw.map((source, index) => {
      const employeeId = clean(fromColumn(source, ['Emp ID', 'Employee ID', 'EMPID']))
      const employeeName = clean(fromColumn(source, ['Name', 'Emp Name', 'Employee Name']))
      const projectName = clean(fromColumn(source, ['Project Name', 'ProjectName']))
      const projectId = clean(fromColumn(source, ['Proj ID', 'Project ID', 'ProjectId']))
      const subProjectId = clean(fromColumn(source, ['Sub-proj ID', 'Sub Project ID', 'SubProjectId']))
      const month = fromColumn(source, ['Month'])
      const amountRaw = fromColumn(source, ['Amount'])
      const errors = []
      if (!employeeId) errors.push('Emp ID is required.')
      if (!employeeName) errors.push('Name is required.')
      if (!projectName) errors.push('Project Name is required.')
      if (!projectId) errors.push('Proj ID is required.')
      if (!subProjectId) errors.push('Sub-proj ID is required.')
  ...
  const importRows = async () => {
    ...
    setImporting(true)
    try {
      await createRecords('otherEntries', valid.map((row) => ({ ...row, errors: [], isValid: true, voucherNo, voucherDate, type: entryType, ledgerDocumentId: ledger.id, ledgerName: ledger.name, ledgerCategoryName: ledger.categoryName, groupName: ledger.groupName, sheetName, sourceFileName: fileName })))
      setComplete(true)
    } catch (requestError) { setError(requestError.message) } finally { setImporting(false) }
  }

  return <Stack spacing={3}>
    <PageHeader section="Imports / Other Entries" title="Other Entries Upload" description="Validate and import forecast or actual ledger amounts." />
    {!activeLedgers.length && !ledgers.loading && <Alert severity="warning">Create an active Ledger in Ledger Master before importing Other Entries.</Alert>}
